...
def test_real_sc0001_readiness_reports_four_blocked_metadata_only_packs() -> None:
    pack_paths = sorted(
        (REPO_ROOT / "visual_dev/elements").glob("**/pack_manifest.yaml")
    )
    before = {path: path.read_text(encoding="utf-8") for path in pack_paths}

    report = audit_pack_lock_readiness(REPO_ROOT, "SC0001")

    assert report["report_status"] == "blocked_missing_canonical_assets"
    assert report["ready_for_human_lock_review"] is False
    assert report["summary"]["total_packs"] == 4
    assert report["summary"]["blocked_packs"] == 4
    assert report["summary"]["packs_missing_canonical_assets"] == 4
    assert all(pack["pack_status"] == "metadata_only" for pack in report["packs"])
    assert all(pack["canonical_asset_count"] == 0 for pack in report["packs"])
    assert all(
        "no canonical image assets found in pack" in pack["missing_requirements"]
        for pack in report["packs"]
    )
    assert all(path.read_text(encoding="utf-8") == before[path] for path in pack_paths)


def test_real_sc0001_report_file_has_no_binary_side_effects(tmp_path: Path) -> None:
    output_path = tmp_path / "SC0001_pack_lock_readiness.yaml"

    write_pack_lock_readiness_report(
        REPO_ROOT,
        "SC0001",
        output_path=output_path,
        report_at="2026-05-04T20:15:00Z",
    )
